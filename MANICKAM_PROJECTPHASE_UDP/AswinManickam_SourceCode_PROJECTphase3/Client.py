...
for i in range(0,loop):                                             #it runs 'loop' times
    ...
    seq_nmbr = Send_Receive.RdtSendPkt(sock,addr,seq_nmbr,ImgPkt,E_Prob)    #calls the function rdt_send to send the packet
    i=i+1                                                           #Loop iteration

f.close()                                                           #File closed
sock.close()                                                        #Socket Closed

end = datetime.datetime.now()                                       #Gets the End time
Elapsed_time = start - end                                          #Gets the elapsed time
print ("Client: File Sent\nFile size sent to server: {0}\nTime taken in Microseconds:{1} ms".format(fileSize,Elapsed_time.microseconds))
