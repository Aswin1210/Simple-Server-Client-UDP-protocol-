...
for i in range(0,loop):                                                     #Loop to write entire transferred image in the new file, it runs 'loop' times
    ...
    ImgPkt,address,receiver_sequence = Send_Receive.RdtReceivePkt(sock,buffer_size,receiver_sequence,E_prob)   #Calls the function RdtReceivePkt to receive the packet
...
end = datetime.datetime.now()                                               #Finding the end-time
Elapsed_time = start - end                                                  #Elapsed time
print ("Server: File Received\nReceived File size: {0}\nTime taken in Microseconds: {1} ms".format(Received_File_Size,Elapsed_time.microseconds))
